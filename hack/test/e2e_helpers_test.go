...
func runDockerRunProxyOnlyE2E(t *testing.T) error {
	t.Helper()
...
`)
	...
	args := []string{
		...
		"sh", "-lc",
		"apk add --no-cache curl >/dev/null && curl -fsS https://httpbin.org/get >/tmp/out && grep -q '\"url\": \"https://httpbin.org/get\"' /tmp/out && echo docker-run-proxy-ok",
	}
	output, err := runKeelCommand(projectDir, artifacts.keelPath, tmpHome, args)
	if err != nil {
		return fmt.Errorf("docker run e2e failed: %w\n%s", err, output)
	}
	...
	for _, want := range []string{
		...
		"Network summary:",
		"dl-cdn.alpinelinux.org:443 policy=allowed",
		"httpbin.org:443 policy=allowed",
	} {
		if !strings.Contains(text, want) {
			return fmt.Errorf("docker run e2e missing %q\n%s", want, text)
		}
	}
	if strings.Contains(text, "policy=denied") {
		return fmt.Errorf("docker run e2e unexpectedly denied traffic\n%s", text)
	}
	return nil
}
